...
int start(const char *out_file_path, const char *pcm_file_path) {
    ...
    in_file = fopen(pcm_file_path, "rb");

    av_register_all();

    //Method 1.
    pFormatCtx = avformat_alloc_context();
    fmt = av_guess_format(NULL, out_file_path, NULL);
    pFormatCtx->oformat = fmt;


    //Method 2.
    //avformat_alloc_output_context2(&pFormatCtx, NULL, NULL, out_file_path);
    //fmt = pFormatCtx->oformat;

    //Open output URL
    if (avio_open(&pFormatCtx->pb, out_file_path, AVIO_FLAG_READ_WRITE) < 0) {
    ...
    av_dump_format(pFormatCtx, 0, out_file_path, 1);
